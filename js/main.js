...
//------------  POLAROID CLICK AND DRAG --------------// AI used to understand, generate and troubleshoot this code, struggle to find  tutorials specific to this idea
...
stack.querySelectorAll('.polaroid').forEach(img => {
  ...
  function start(e) {
    ...
    const newPosition = e.touches ? e.touches[0] : e;
    offX = newPosition.clientX - img.offsetLeft;
    offY = newPosition.clientY - img.offsetTop;
  ...
  function move(e) {
    if (!dragging) return;
    const newPosition = e.touches ? e.touches[0] : e;
    img.style.left = (newPosition.clientX - offX) + 'px';
    img.style.top = (newPosition.clientY - offY) + 'px';
  }
...
//-----ANIMATED TIMELINE SCROLL------------// AI used to generate and problem solve parts of this code
...
const observerTag = new IntersectionObserver((galleryTag) => {
  ...
  galleryTag.forEach((enterTag) =>{
    ...
    if (enterTag.isIntersecting) {
      ...
      enterTag.target.classList.remove('showtag');
    }
  });
});
